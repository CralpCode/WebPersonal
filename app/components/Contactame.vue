    ...
    <div class="w-11/12 sm:w-11/12 md:w-11/12 lg:w-6xl xl:w-6xl h-full mx-auto py-10">
        ...
        <UPageCard title="" description="" icon="" spotlight
            ...
            <div
                ...
                <UForm :validate="validate" :state="state" class="space-y-4 w-full max-w-lg mx-auto" @submit="onSubmit">

                    <input v-model="state._gotcha" type="text" name="_gotcha" class="opacity-0 absolute -z-10 w-0 h-0"
                        tabindex="-1" autocomplete="off" />

                    <UFormField label="Nombre" name="name" class="w-full">
                        <UInput v-model="state.name" placeholder="Tu nombre" icon="i-lucide-user" class="w-full" />
                    </UFormField>

                    <UFormField label="Email" name="email" class="w-full">
                        <UInput v-model="state.email" placeholder="tucorreo@ejemplo.com" icon="i-lucide-mail"
                            class="w-full" />
                    </UFormField>

                    <UFormField label="Asunto" name="subject" class="w-full">
                        <UInput v-model="state.subject" placeholder="Interés en colaborar..." icon="i-lucide-tag"
                            class="w-full" />
                    </UFormField>

                    <UFormField label="Mensaje" name="message" class="w-full">
                        <UTextarea v-model="state.message" placeholder="Hola, me gustaría hablar sobre..." :rows="4"
                            class="w-full" />
                    </UFormField>

                    <div class="pt-2">
                        <UButton type="submit" block :loading="loading" :disabled="cooldownTime > 0"
                            :icon="cooldownTime > 0 ? 'i-lucide-timer' : 'i-lucide-send'"
                            :color="cooldownTime > 0 ? 'neutral' : 'primary'" variant="solid">
